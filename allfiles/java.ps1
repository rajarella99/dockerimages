...
function jreinstall {
    ...
    if ($null -eq (Get-Command cmd -ErrorAction SilentlyContinue)) {
        ...
        #Start-Process "start /w C:\jre8u351.exe INSTALL_SILENT=Enable REBOOT=Disable SPONSORS=Disable"
        #Start-Process 'cmd' -Verb runAs -ArgumentList "/C $arguments"
        ...
        Write-Host "The Following "$javaversion[0].tostring()" is installed using function"        
    }
}
...
if (Test-Path -Path $Folder) {
    ...
    $javaversion = & cmd /c "java -version 2>&1"
    Write-Host -ForegroundColor Magenta "The Following "$javaversion[0].tostring()" is installed"
