...
echo "Signing the Otel Collector Contrib Executable"
osslsigncode sign -pkcs12 "$signpath" -pass "apple@123" -n "opentelemetry contrib collector executable signing" -t "http://timestamp.digicert.com" -verbose -h sha256 -in "$exepath" -out "$PWD/otelcontribcol_windows_amd64-${today}.exe"
else
