...
#$encrypted = ConvertFrom-SecureString -SecureString $acrstring
$linuxfxversion = "DOCKER|wkmarcosyard.azurecr.io/linux/telemetry2:0.34.0"
$templatefile = "D:\wkmarcos\allfiles\linuxcontainertemplate.json" ## you'll need to provide this json template absolute path in this variable here from wherever you're calling this ARM file using script##
...
<# Working Code with No LinuxFxVersion from Microsoft support #
...
## LinuxFxVersion Missing  #>
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++#

## Create a Linux WEB APP on AZURE APP SERVICE using ARM Templates ##
try {
if ($Kind -ne "windows") {
Write-Host -ForegroundColor Green "checking if "$Kind" web app can be created"
}
...
$planstatus = Get-AzAppServicePlan -ResourceGroupName $RGName -Name $serviceplanname 
if ( $planstatus.Kind -eq "linux") {
Write-Host -ForegroundColor Green "The "$Kind" Web AppService creating using ARM Template"
$webparams = @{
    subscriptionId = $SubscriptionId
    name = $webappname
    location = $location
    hostingPlanName = $serviceplanname
    serverFarmResourceGroup = $RGName
    alwaysOn = $True
    linuxFxVersion = $linuxfxversion
    dockerRegistryUrl = $acrregistryname
    dockerRegistryUsername = $acrloginserver
    dockerRegistryPassword = $acrstring
...
$deployment = @{
    ...
    TemplateParameterObject = $webparams
    Verbose = $True
}
New-AzResourceGroupDeployment @deployment
Write-Host -ForegroundColor Yellow "The "$Kind" Web AppService created successfully using ARM Template "
Start-Sleep -Seconds 10 -Verbose
#hashtable#
	$sitesettings = @{
		ResourceGroupName  =		$RGName		
