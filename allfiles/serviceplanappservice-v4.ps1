## Provision Azure Service Plan & Azure App Services ##additional version4 using Azcli and Azure powershell cmdlets##needs powershell core  or powershell v5 to run##
[CmdletBinding()]
Param (
...
$acrwindowsimagename = "$acrloginserver.azurecr.io/$Kind/dec2windowsotel38:0.38.0"
$acrlinuximagename = "$acrloginserver.azurecr.io/$Kind/dec2linuxotel40:0.40.0"	
...
#az webapp config container set --docker-custom-image-name $acrlinuximagename --docker-registry-server-password $acrstring --docker-registry-server-url $acrregistryname --docker-registry-server-user $acrloginname --name $webappname --resource-group $RGName --verbose
az webapp config container set --docker-custom-image-name $acrlinuximagename --docker-registry-server-password $acrstring --docker-registry-server-url $acrregistryname --docker-registry-server-user $acrloginserver --name $webappname --resource-group $RGName --verbose
